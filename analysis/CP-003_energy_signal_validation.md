...
Status: protocol prepared by Codex on 2026-08-31; no vehicle evidence collected yet.
...
- Captures store aggregate statistics, not a raw sample stream: 5 Hz for the first minute, then
  ...
  JSON summaries; only the eight newest are retained. One full six-capture run fits below that
  ceiling. Pull and hash all files before another vehicle/generation run.
- Optional diagnostic context may be exported separately through the parked-only USB action.
  It never substitutes for the evidence JSON required below.
...
| EVChargePilot APK filename/version | pending |
| APK SHA-256 | pending |
| APK signing certificate SHA-256 | pending |
...
Run each scenario as a separate capture. Start parked. Return to a safe parked state before
stopping and saving. A skipped scenario is explicitly `not tested`.
...
4. **Motorway steady speed — 10 minutes when legal and safe.** Hold approximately 110 km/h for
   5 minutes, then approximately 130 km/h for 5 minutes. If either speed is illegal or unsafe,
   omit it. One combined capture is sufficient for availability/cadence; it is not model-fitting
   data because this recorder intentionally retains no raw track.
5. **Grade — one downhill and one uphill segment.** Use an existing safe route; do not seek a
...
The saved absolute path is shown in-app. From an already-authorized ADB workstation, pull that
exact path without changing app or filesystem permissions:
...
| pending | motorway | pending | pending | not tested |
| pending | grade | pending | pending | not tested |
